...
type UIElements = {
  ...
  top5List: HTMLElement;
  manualActionsPanel: HTMLElement;
  startIslands: HTMLInputElement;
  mergeEvery: HTMLInputElement;
  // topKFromIsland: HTMLInputElement;
  // keepTopTotal: HTMLInputElement;
};
...
class Island {
  ...
  steps: number;
  bestFitness: number;
  bestSequence: Uint32Array | null;

  constructor(size: number, steps: number, actionsCount: number) {
    this.size = size;
    this.steps = steps;
    this.sequences = new Uint32Array(size * steps).map(() => 
  ...
  updateFitness(fitnessArray: number[], startIdx: number) {
  ...
  getTopSequences(k: number): {seq: Uint32Array, fit: number}[] {
  ...
  evolve(mutationRate: number, eliteFrac: number, actionsCount: number) {
    ...
    for (let i = 0; i < eliteCount; i++) {
      ...
      newSequences.set(
        this.sequences.slice(idx * this.steps, (idx + 1) * this.steps),
        i * this.steps
      );
    }
...
function calculateMatchPercentage(gridA: Uint8Array, gridB: Uint8Array | null): number {
    ...
    if (gridA.length !== size || gridB.length !== size) return 0;
    let matches = 0;
...
function getUIElements(): UIElements {
  return {
    ...
    demoActionsCanvas: document.getElementById('demoActionsCanvas') as HTMLElement,
    demoPlaybackLabel: document.getElementById('demoPlaybackLabel') as HTMLElement,
    ...
    top5List: document.getElementById('top5List') as HTMLElement,
    manualActionsPanel: document.getElementById('manualActionsPanel') as HTMLElement,
    startIslands: document.getElementById('startIslands') as HTMLInputElement,
    mergeEvery: document.getElementById('mergeEvery') as HTMLInputElement,
...
function formatSequence(seq: Uint32Array, maxLen: number = 12): string {
...
function updateTop5Display(ui: UIElements) {
  ...
  for (let gen = 0; gen < generations && isRunning; gen++) {
    ...
    for (const island of islands) {
      for (let i = 0; i < island.size; i++) {
        ...
        const seqStr = Array.from(
          island.sequences.slice(i * steps, (i + 1) * steps)
        ).join(',');
        seqSet.add(seqStr);
    ...
    for (const island of islands) {
      ...
      for (const {seq, fit} of topFromIsland) {
        if (fit > 0) {
          ...
          const exists = top5Sequences.find(e => 
            Array.from(e.sequence).join(',') === seqKey
          );
    ...
    if (shouldMerge) {
      ...
      const candidates: {seq: Uint32Array, fit: number}[] = [];
      for (const island of islands) {
        // added 
...
function generateActionButtons(container: HTMLElement, clickHandler?: (actionIndex: number) => void) {
